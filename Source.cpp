...
{
    if (!gImage && !gAORM && !gUnityMask) return;

    Bitmap* bmp = gChannelImage ? gChannelImage : (gAORM ? gAORM : gUnityMask ? gUnityMask : (Bitmap*)gImage);
    ...
    WCHAR suffix[16] = L"_original";
    if (gLastChannel == 0) wcscpy_s(suffix, L"_red");
    if (gLastChannel == 1) wcscpy_s(suffix, L"_green");
    if (gLastChannel == 2) wcscpy_s(suffix, L"_blue");
    if (gLastChannel == 3) wcscpy_s(suffix, L"_alpha");
    if (gAORM) wcscpy_s(suffix, L"_aorm");
    if (gUnityMask) wcscpy_s(suffix, L"_unitymap");
